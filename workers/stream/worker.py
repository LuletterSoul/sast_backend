...
class _BaseStreamer(object):
    ...
    def _input(self, batch) -> int:
        ...
        # future = Future(task_id, 1, weakref.ref(self._future_cache))
        ...
        for model_input in batch:
            ...
            request_id += 1
        # self._send_request(task_id, request_id, batch)
        # request_id += 1
        return task_id
    ...
    def predict(self, model_input):
        task_id = self._input(model_input)
...
class _BaseStreamWorker(object):
    ...
    def model_predict(self, *args):
        batch_result = self._predict(*args)
        return batch_result
    ...
    def _run_once(self):
        ...
        #     model_output = self.model_predict(item[3])
        #     self._send_response(client_id, task_id, request_id, model_output)
...
def _setup_redis_worker_and_runforever(model_class, batch_size, max_latency, gpu_id, redis_broker, prefix='',
                                       model_init_args=None, model_init_kwargs=None, destroy_event=None):
...
def run_redis_workers_forever(model_class, batch_size, max_latency=0.1,
                              ...
                              prefix='', model_init_args=None, model_init_kwargs=None, destroy_event=None):
    procs = []
...
#
# class _DynamicBaseStreamWorker(object):
#     """
#     This a little modification class of _BaseStreamWorker in `service_streamer` library.
#     When need predict function receive multiple paired inputs instead of a simple batch input,
#     so we change `model_input(batch_input)` into `model_input(*args)`
#     """
#
#     def __init__(self, predict_function, batch_size, max_latency, *args, **kwargs):
#         super().__init__()
...
#
#
# class StreamWorker(_DynamicBaseStreamWorker):
#     def __init__(self, predict_function_or_model, batch_size, max_latency, request_queue, response_queue,
#                  model_init_args, model_init_kwargs, *args, **kwargs):
#         super().__init__(predict_function_or_model, batch_size, max_latency, *args, **kwargs)
#         self._request_queue = request_queue
...
#
#
# class RedisWorker(_DynamicBaseStreamWorker):
#     def __init__(self, model_class, batch_size, max_latency=0.1,
#                  redis_broker="localhost:6379", prefix='',
#                  model_init_args=None, model_init_kwargs=None, *args, **kwargs):
#         # assert issubclass(model_class, ManagedModel)
